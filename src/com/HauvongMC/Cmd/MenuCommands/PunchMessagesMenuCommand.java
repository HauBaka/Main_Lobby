...
public class PunchMessagesMenuCommand implements CommandExecutor {
    @Override
    public boolean onCommand(CommandSender sender, Command command, String label, String[] args) {
        if (label.equalsIgnoreCase("PunchMessagesMenu")) {
            if (!(sender instanceof Player)) return false;
            if (!(((Player) sender).getWorld().getName().equalsIgnoreCase("world"))) {
                sender.sendMessage("§cLệnh chỉ có thể sử dụng ở sảnh!");
                return false;
            }
            PunchMessagesMenu.openMain(((Player) sender).getPlayer(), false);
            return true;
        }
        return true;
    }
